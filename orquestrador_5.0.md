...
- Mensagem atual (pode ser a conversa consolidada até aqui):
```text
{{ $json.mensagem }}
```
...
- Termos de risco (agravamento súbito, desmaio, dor intensa, sangramento, etc.).
...
#### 5.1 Condições estruturais obrigatórias
...
Além disso, só considere **Aguardando agendamento** quando a conversa indicar que:
...
- Nesse caso:
  ...
  - Se já há nome real, mas sem qualificação/valor/disponibilidade claros → Regra 4 (Qualificação).

#### 5.2 Gatilhos de linguagem

Use **Aguardando agendamento** quando, **além** de atender às condições estruturais acima, a mensagem indicar prontidão clara, por exemplo:

**Gatilhos inequívocos (com lead já qualificado, valor explicado e disponibilidade mínima conhecida):**
...
- Se `lead_info.qualificado = true` **e** as condições de 5.1 (nome real, qualificação, valor explicado e alguma disponibilidade) já estiverem satisfeitas, você pode tratar esses gatilhos ambíguos como prontidão e enviar para Aguardando agendamento.  
...
```json
{
  ...
  "rationale": "Lead já está qualificado, viu o valor da consulta, informou disponibilidade e expressou prontidão clara para marcar.",
  "updates": []
}
```
